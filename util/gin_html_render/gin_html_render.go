...
func (r *Render) Create() *Render {
	...
	// root dir
	tplRoot, err := filepath.Glob(r.TemplatesDir + "*")
	if err != nil {
		panic(err.Error())
	}

	// sub dirs
	tplSub, err := filepath.Glob(r.TemplatesDir + "**/*")
	if err != nil {
		panic(err.Error())
	}
	rTpl, _ := regexp.Compile(`.+\.tpl$`)
	rHtml, _ := regexp.Compile(`.+\.html$`)
	for _, tpl := range append(tplRoot, tplSub...) {
		if !rTpl.MatchString(tpl) && !rHtml.MatchString(tpl) {
			continue
		}
		...
		if rTpl.MatchString(tpl) {
			r.AddFromFiles(name, layout, tpl)
		} else if rHtml.MatchString(tpl) {
...
func (r *Render) Validate() {
	...
	layoutFile := r.TemplatesDir + r.Layout
